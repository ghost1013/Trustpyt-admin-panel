...
const AddCompanyForm = ({categories}) => {

    const [company, setCompany] = useState({name: '', category_id: '', sub_category_id: '', logo: '', verified: false, website: ''})
  ...
  return (
    ...
    <InputLabel>Logo</InputLabel>
        ...
        <h4>{progress}%</h4>
        <img src={url} alt="Wait for preview" width="200px" />
    </Box>
   </Box>
   <Box sx={{display: 'flex',justifyContent: 'space-between', px: 4, pb:2}}>
   <Box sx={{width: '40%'}}>
   ...
   <Box sx={{display: 'flex', justifyContent: 'flex-end', px: 4, pb: 3}}>
        <Button 
        onClick={() => onAddition({data: company, logo: url})}
